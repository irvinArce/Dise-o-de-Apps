...
export default function Almacenamiento({route, navigation}){

    const { userData } = route.params;
    const { formData } = route.params;

    const url = "https://pcbuilder4a.000webhostapp.com/api-rest/Componentes/getAllComponentes.php?componente=almacenamiento&gama="+formData.gama;
    ...
    const onSubmit = (data) => {
        ...
        navigation.navigate('Refrigeracion', { formData: array , userData: userData});
    };
                ...
                renderItem={({ item }) => (
                    <View style={styles.radio}>
                        <RadioButton.Item
                            label={item.nombre}
                            value={item.numero}
                            onPress={() => setValue("almacenamiento", item.numero)}
                            status={watch("almacenamiento") === item.numero ? 'checked' : 'unchecked'}
                        />
                    </View>
                )}
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.Title}>Selecciona el Almacenamiento</Text>
            {getContent()}
        </View>
    );
}
