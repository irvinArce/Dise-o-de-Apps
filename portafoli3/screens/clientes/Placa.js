...
export default function Placa({route, navigation}){

    const { userData } = route.params;
    const { formData } = route.params;

    const url = "https://pcbuilder4a.000webhostapp.com/api-rest/Componentes/getAllComponentes.php?componente=placaMadre&gama="+formData.gama;
    ...
    const onSubmit = (data) => {
        ...
        navigation.navigate('Almacenamiento', { formData: array , userData: userData});
    };
                ...
                renderItem={({ item }) => (
                    <View style={styles.radio}>
                        <RadioButton.Item
                            label={item.nombre}
                            value={item.numero}
                            onPress={() => setValue("placa", item.numero)}
                            status={watch("placa") === item.numero ? 'checked' : 'unchecked'}
                        />
                    </View>
                )}
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.Title}>Selecciona la Placa Madre</Text>
            {getContent()}
        </View>
    );
}
