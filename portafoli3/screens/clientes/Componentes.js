...
export default function Componentes({route, navigation}){

    const { userData } = route.params;
    const { formData } = route.params;

    const url = "https://pcbuilder4a.000webhostapp.com/api-rest/Componentes/getAllComponentes.php?componente=cpu&gama="+formData.gama;
    ...
    const onSubmit = (data) => {
        ...
        console.log(array)
        navigation.navigate('Gpu', { formData: array , userData: userData});
    };
                ...
                renderItem={({ item }) => (
                    <View style={styles.radio}>
                        <RadioButton.Item
                            label={item.nombre}
                            value={item.numero}
                            onPress={() => setValue("cpu", item.numero)}
                            status={watch("cpu") === item.numero ? 'checked' : 'unchecked'}
                        />
                    </View>
                )}
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.Title}>Selecciona el CPU</Text>
            {getContent()}
        </View>
    );
}
