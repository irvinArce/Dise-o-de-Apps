...
export default function Gpu({route, navigation}){

    const { userData } = route.params;
    const { formData } = route.params;

    const url = "https://pcbuilder4a.000webhostapp.com/api-rest/Componentes/getAllComponentes.php?componente=gpu&gama="+formData.gama;
    ...
    const onSubmit = (data) => {
        ...
        navigation.navigate('Gabinete', { formData: array , userData: userData});
    };
                ...
                renderItem={({ item }) => (
                    <View style={styles.radio}>
                        <RadioButton.Item
                            label={item.nombre}
                            value={item.numero}
                            onPress={() => setValue("gpu", item.numero)}
                            status={watch("gpu") === item.numero ? 'checked' : 'unchecked'}
                        />
                    </View>
                )}
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.Title}>Selecciona el GPU</Text>
            {getContent()}
        </View>
    );
}
