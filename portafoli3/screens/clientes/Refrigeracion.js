...
export default function Refrigeracion({route, navigation}){
    ...
    const url = "https://pcbuilder4a.000webhostapp.com/api-rest/Componentes/getAllComponentes.php?componente=refrigeracion&gama="+formData.gama;
                ...
                renderItem={({ item }) => (
                    <View style={styles.radio}>
                        <RadioButton.Item
                            label={item.nombre}
                            value={item.numero}
                            onPress={() => setValue("refrigeracion", item.numero)}
                            status={watch("refrigeracion") === item.numero ? 'checked' : 'unchecked'}
                        />
                    </View>
                )}
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.Title}>Selecciona la Refrigeracion</Text>
            {getContent()}
        </View>
    );
}
